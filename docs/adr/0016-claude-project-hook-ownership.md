...
decision. [ADR 0017](0017-claude-project-hook-transaction.md) separately accepts
the later generated format, receipt, and transaction design without enabling it.
...
The root-bound launch and receipt format is not authorized by this ADR. ADR 0017
now defines that later design, but its dependency-ordered implementation slices
remain unimplemented. Until invocation verification exists, the fixture adapter
always records `unknown`.
...
ADR 0017 defines the eventual receipt-aware ownership and artifact-readiness
states, canonical generated bytes, root binding, invocation arguments, and
recovery. Those states remain unimplemented and this preflight must not reserve
meaning by pretending its artifacts already exist.

### Acceptance criteria for the implemented read-only slice

The implemented read-only `agenthawk integrations claude status` slice must:
...
No install/remove command is included. ADR 0017 covers the required root-binding
format, exact owned bytes, no-replace publication capability, cancellation
linearization, recovery ordering, invocation-time verification, and exact-host
discovery as design only.
