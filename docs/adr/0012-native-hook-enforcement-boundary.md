...
The first named run used the official `@openai/codex@0.149.0` Windows x64 npm artifact on 2026-08-23. The denial hook path passed and the fake package-manager marker remained absent. The neutral hook returned control to Codex, but Codex's workspace sandbox rejected its cached PowerShell runtime before the benign command could run. The harness therefore failed with `neutral_command_failed:sandbox_rejected:denial_passed`; this is partial compatibility evidence, not a supported-adapter result. The local CLI remains unsupported, and no desktop, IDE, remote, or cloud claim follows from this run.
