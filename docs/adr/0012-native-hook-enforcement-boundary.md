...
Research for this decision used current primary vendor documentation, accessed 2026-08-21, rechecked for the Codex implementation slice on 2026-08-23, and rechecked for the Claude Code proposal on 2026-08-24:
...
| Claude Code | `PreToolUse` receives distinct `Bash` and `PowerShell` inputs; neutral silence preserves normal permissions, structured deny or exit `2` denies | user, project, local, plugin, session, SDK, and managed sources; interactive hooks wait for current-folder or inherited parent trust, while `-p`/SDK has no trust dialog; `disableAllHooks`, bare mode, source selection, and managed-only policy can exclude hooks | matching hooks run in parallel and deny takes precedence; command-hook startup failure, non-2 error, malformed output, and timeout are non-blocking; default command timeout is 600 seconds; commands run with user permissions and inherit the parent environment except documented removals | [ADR 0015](0015-claude-code-hook-edge.md) pins the `2.1.241` fixture edge; [ADR 0016](0016-claude-project-hook-ownership.md) defines read-only status and an exclusive local-settings candidate; no supported row exists |
...
5. separate Claude Code, Cursor, and GitHub Copilot adapters only after their exact contracts have independent fixtures and failure tests. [ADR 0015](0015-claude-code-hook-edge.md) implements the closed Claude fixture edge, while [ADR 0016](0016-claude-project-hook-ownership.md) authorizes only its read-only status design.

The research slice itself installed no hook. Collision-safe `init` remains an advisory-template initializer. [ADR 0013](0013-codex-project-hook-ownership.md) accepts and implements a separately requested project-scoped exclusive-ownership lifecycle. Its isolated exact-host project discovery, trust, execution, mutation, feature-disable, and managed-only exact-artifact evidence passes. [ADR 0014](0014-codex-support-scope.md) keeps the candidate unsupported: its complete local ordinary-CLI matrix passes, but the pinned hosted Windows environment rejects the restricted-token split writable-root projection before execution.
...
Support is decided per exact named surface, version, operating system, sandbox configuration, and deployment authority; evidence from one row never unlocks another. Local CLI evidence cannot be generalized to app-server, SDK, IDE, desktop, Remote, cloud, managed requirements, another operating system, or another version. The project-hook ownership lifecycle, isolated manual activation, and managed-only exact-artifact boundary pass under [ADR 0013](0013-codex-project-hook-ownership.md). [ADR 0014](0014-codex-support-scope.md) withholds the user-facing Codex CLI row after the fresh hosted Windows boundary rejected the restricted-token split writable-root projection before execution. Its `portable` qualifier makes no target-shell claim and intentionally rejects shell-specific syntax; unknown shell constructs can still conceal dependency operations and remain a residual bypass. [ADR 0015](0015-claude-code-hook-edge.md) pins the closed Claude Code `2.1.241` fixture edge, and [ADR 0016](0016-claude-project-hook-ownership.md) accepts only its collision preflight design. Both `Bash` and `PowerShell` rows remain unsupported pending status, root-bound ownership, activation, and real-host evidence. Each later adapter and surface requires equivalent independent evidence. A vendor without a proven neutral no-auto-approve result remains unsupported; a cloud surface without required provider reachability or a visible warning channel must deny rather than silently degrade.
...
The exact-version local app-server stdio slice passed on Windows with the official `@openai/codex@0.149.0` artifact. Its bounded JSONL client omits JSON-RPC decoration as required by the pinned protocol, rejects malformed UTF-8, duplicate/trailing JSON through the production strict parser, oversized output, unknown or duplicate response identifiers, server requests, approval requests, and unfinished process cleanup. Each neutral and denial scenario uses a fresh temporary `CODEX_HOME`, Git root, process, and loopback provider. The original harness inventories exactly one temporary user `PreToolUse` command hook, writes trust only for that hook's listed key and current hash through `config/batchWrite`, verifies the resulting trusted hash, starts an ephemeral `workspace-write`/`approvalPolicy: never` thread, and binds the matching `hook/started`, `hook/completed`, and `turn/completed` notifications. The project activation extension instead installs the ADR-owned root-bound pair, requires Codex's exact `project` source, observes initial distrust and exact trust, proves neutral and denied execution, observes definition mutation in both Codex and AgentHawk, and proves disabled hooks produce an empty inventory with no provider request. It does not use the broadly trusting session flag or the unsandboxed `thread/shellCommand`, `command/exec`, or `process/*` methods. The named `local-app-server-windows-stdio-shell-command` surface passed independent neutral regular-file evidence and blocked-hook plus absent-denial-marker evidence. This protocol result is not evidence for VS Code, desktop, Remote, cloud, Linux, macOS, or another version. Separate disposable hosted gates prove managed-only suppression and an exact restricted-token split writable-root exclusion for the same artifact; both remain compatibility/exclusion evidence and do not create a supported project row.
