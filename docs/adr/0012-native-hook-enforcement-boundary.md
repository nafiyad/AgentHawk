...
| Cursor | prefer narrow `beforeShellExecution`, with broader `preToolUse` evaluated separately; structured deny or exit `2` denies | project, user, system/MDM, team, and enterprise cloud sources; enterprise has higher source priority and hooks are merged | failures are fail-open unless `failClosed: true`; the timeout must be set explicitly; cloud loads project/team/enterprise command hooks but not user hooks, and early read-only cloud turns do not run hooks; inputs/environment can expose workspace, transcript, session, and user data | documentation fixture only; no supported AgentHawk adapter/version yet |
| GitHub Copilot | `preToolUse` has distinct camelCase CLI/cloud and PascalCase compatibility payload forms; structured deny controls the call | CLI can combine machine policy, repository, user, inline, and plugin hooks; policy hooks require administrator installation and cannot be disabled normally; cloud has repository hooks only | matching hooks run in order and any deny wins; command-hook nonzero failures deny for this event, but timeouts are always fail-open, including policy hooks; default timeout is 30 seconds; cloud hook processes can receive Copilot API/Git credentials and prompt data that AgentHawk must ignore | documentation fixture only; no supported AgentHawk adapter/version yet |
...
- `unrelated`: neutral result and zero policy, cache, or provider calls;
...
No-operand install/restore commands are not dependency-add admissions. Package-manager calls hidden inside scripts, interpreters, nested shells, child processes, aliases, functions, build tools, or manually opened terminals are unsupported. Manifest edits remain covered by post-edit `scan`/`diff` and protected CI; a post-edit check is not described as pre-action prevention.
...
Milestone 17 needs contract and qualifier code before a vendor adapter. Each adapter requires pinned payload fixtures, golden neutral/deny output, process failure and timeout observations, no-provider unrelated tests, packed-package consumer tests, and Windows/macOS/Linux coverage where the host exists.
