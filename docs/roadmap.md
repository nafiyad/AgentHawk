...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, and bounded aggregate evaluator are complete. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate now has a dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and a development-only isolated real-host harness. The first exact `@openai/codex@0.149.0` Windows CLI run proved the denial path without executing the fake package manager, but its neutral command was rejected by Codex's workspace sandbox at the cached PowerShell runtime boundary. The harness correctly failed and no support claim was made. The immediate next action remains the same Codex slice: reproduce or resolve that host sandbox boundary, then obtain complete neutral/deny evidence across named local and remote surfaces. Only after that evidence passes may a separate ownership-reviewed slice update the support matrix and installation guidance. Do not begin Claude Code while Codex compatibility remains unproven.
