...
2. Claude Code `PreToolUse` adapter and fixtures. [ADR 0015](adr/0015-claude-code-hook-edge.md) completes the exact-contract research proposal; fixture implementation is next.
...
Milestone 16 is complete. Milestone 17's vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, and unsupported Codex compatibility candidate are complete. [ADR 0015](adr/0015-claude-code-hook-edge.md) now completes the independent Claude Code research gate against official documentation and the exact `2.1.241` release. It records distinct `Bash`/`PowerShell` inputs, neutral-silence and deny-only output, interactive current-folder/inherited-parent trust versus dialog-free `-p`/SDK behavior, full-user/environment authority, parallel siblings, managed/bare/source exclusions, and non-blocking startup, malformed-output, and timeout failures. No Claude adapter or configuration was installed, and the locally discovered `2.1.141` npm-based executable creates no compatibility evidence. Every native row remains unsupported. The next dependency-ordered slice is the closed Claude `PreToolUse` fixture adapter and adversarial process tests; configuration ownership, installation, activation, and real-host matrices remain later independent gates.
