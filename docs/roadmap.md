...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, and bounded aggregate evaluator are complete. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate now has a dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and a development-only isolated real-host harness. Exact-source review corrected the first Windows result: the temporary configuration had omitted the native Windows sandbox backend, so Codex rejected the neutral command by policy before process creation. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact `@openai/codex@0.149.0` local Windows CLI run now proves both neutral repository execution and denial without executing the fake package manager. The harness now names Windows, Linux, and macOS CLI surfaces separately, rejects unknown platforms, and requires a regular repository-local marker for every neutral proof. An integrity-pinned Linux x64 attempt inside a digest-pinned Docker image failed closed because Codex's bundled `bwrap` could not create its sandbox namespace; no container or Codex security boundary was weakened, so Linux remains unproven. Evidence is accepted per exact named surface rather than requiring or implying undocumented cloud parity. The immediate next action is an exact-version local app-server stdio harness using the same isolated provider; passing it will not establish IDE, desktop, Remote, cloud, managed-requirements, another operating system, or another version. Installation guidance still requires a separate ownership review. Do not begin Claude Code while the selected Codex compatibility scope remains unfinished.
