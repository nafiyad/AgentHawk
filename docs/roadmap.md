...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, [Codex project-hook ownership decision](adr/0013-codex-project-hook-ownership.md), and root-bound artifact-format prerequisite are complete. The pure format binds one deterministic synchronous hook and strict path-redacted receipt to the exact canonical repository identity with 256-bit identifiers, an unambiguous fixed-vector digest, exact artifact digests, bounded shell-safe command quoting, and strict launch declarations; it does not write or activate configuration. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate otherwise retains its dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and development-only isolated real-host harnesses. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact official Windows artifact passes both the local CLI and local app-server stdio `shell_command` surfaces. An integrity-pinned Linux x64 Docker attempt failed closed because Codex's bundled `bwrap` could not create its sandbox namespace; no security boundary was weakened. IDE, desktop, Remote, cloud, managed requirements, macOS, other versions, and other vendors remain unproven. Pinned source also shows that Codex redirects linked-worktree hooks to the root checkout, so the first installer must reject linked worktrees rather than claim an inert local declaration. The active action is the smallest read-only project-scoped slice: fixed-target `integrations codex status`, strict ownership/readiness/blocker classification, bounded redacted output, linked-worktree detection, and packed-consumer proof. Transactional `install`/`remove`, invocation verification, recovery, and exact-version activation remain the following slices. Do not begin Claude Code or describe Codex native enforcement as supported until that implementation is independently reviewed and tested.
