...
- For every supported adapter, an actual integration harness proves: benign explicit add can proceed; warn denies visibly unless that exact host first proves a neutral visible-warning channel; review/block/error deny under strict enforcement; malformed hook input denies or errors visibly; an unrelated command performs zero provider calls.
...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, [Codex project-hook ownership decision](adr/0013-codex-project-hook-ownership.md), root-bound artifact format, fixed-target status, collision-safe install, exact-owned removal, interruption recovery, and invocation-time pair verification are complete. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate retains its dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and development-only isolated real-host harnesses. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact official Windows artifact passes both the local CLI and local app-server stdio `shell_command` surfaces. The isolated project-hook harness uses the real installer and proves installed-untrusted discovery, exact-definition trust, neutral execution, dependency-add denial with no fake package-manager execution, mutation to Codex `modified` and AgentHawk `owned_modified`, and feature-disabled empty discovery with zero provider traffic. The checksum-verified official `0.149.0` Windows package also passes a disposable GitHub-hosted managed-only gate: exact project discovery precedes policy, the app-server reports `allowManagedHooksOnly: true`, the non-managed project inventory becomes empty, and provider traffic remains zero. Its policy transaction is atomic no-replace and identity-fenced. [ADR 0014](adr/0014-codex-support-scope.md) keeps all native adapters unsupported because the real-host allow/warn/review/block/error matrix and ordinary CLI project-hook launch are incomplete; exact tagged source also requires the eligible row to exclude Code Mode paths that do not expose a pre-tool payload. The next dependency-ordered slice is the exact Windows `0.149.0` CLI project-hook outcome matrix defined by that ADR. IDE, desktop, Remote, cloud, other operating systems/versions, and other vendors remain separate.
