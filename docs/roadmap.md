...
Milestone 16 is complete. Milestone 17's vendor-neutral contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, unsupported Codex compatibility candidate, closed Claude `2.1.241` fixture edge, read-only Claude project-settings preflight, root-bound lifecycle design, and pure canonical settings/receipt/root-binding format are complete. The format has fixed Windows/POSIX vectors and is not connected to status or mutation. `agenthawk integrations claude status` still reports only minimum-disclosure collision observations and activation `unproven`; it never mutates settings, parses foreign local settings, or creates a support claim. ADR 0017 orders the remaining Claude work: invocation verification, receipt-aware status, transaction implementation, then independent real-host support evidence. The next slice is invocation-time project verification and fail-closed process testing; no install/remove implementation is yet authorized.
