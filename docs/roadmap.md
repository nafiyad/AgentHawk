...
2. Claude Code `PreToolUse` adapter and fixtures. [ADR 0015](adr/0015-claude-code-hook-edge.md) completes the exact-contract research proposal. The closed fixture edge is implemented with a dedicated binary, strict `Bash`/`PowerShell` framing, deny-only output, adversarial process tests, and packed-consumer verification. [ADR 0016](adr/0016-claude-project-hook-ownership.md) and `integrations claude status` add the read-only project-settings boundary. [ADR 0017](adr/0017-claude-project-hook-transaction.md) fixes the root-bound receipt, generated settings, invocation-verification, transaction, recovery, and support boundaries; its pure canonical format, invocation-verification, and receipt-aware status slices are implemented, while configuration mutation and real-host evidence remain separate dependency-ordered gates.
...
Milestone 16 is complete. Milestone 17's vendor-neutral contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, unsupported Codex compatibility candidate, closed Claude `2.1.241` fixture edge, project-settings preflight, root-bound lifecycle format, invocation verification, and receipt-aware status are complete. The binary assigns project trust only after the exact pair, absent lock, current artifacts, and matching identity-fenced snapshots verify; failure denies before providers. `agenthawk integrations claude status` now classifies the root-bound pair as absent, owned exact/inactive/modified, unowned, colliding, or unsafe; reports artifact readiness independently; derives only a canonical lock's single staging target; and aggregates ignore state without exposing paths, identifiers, digests, settings, or ignore sources. Activation remains `unproven`, every Claude support row remains unsupported, and status performs no mutation or provider access. ADR 0017 orders the remaining work: install/remove transaction implementation, then independent real-host support evidence. The next slice is the transaction implementation with actual-filesystem no-replace capability tests.
