...
Milestone 16 is complete. Milestone 17's vendor-neutral contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, unsupported Codex compatibility candidate, and closed Claude `2.1.241` fixture edge are complete. [ADR 0016](adr/0016-claude-project-hook-ownership.md) now accepts the Claude settings collision and minimum-disclosure status design without authorizing mutation. A future candidate may exclusively own only a previously absent `.claude/settings.local.json` after Git proves the exact path is ignored; it will never merge into maintainer-owned shared or local settings. Read-only preflight observes that ignore precondition plus shared `PreToolUse` siblings and `disableAllHooks` declarations as blockers. Repository-only observation cannot prove merged effective activation, so activation remains explicitly `unproven` and every native row remains unsupported. The next dependency-ordered slice is read-only `agenthawk integrations claude status`; install, remove, invocation-time binding, activation, and real-host matrices remain later independent gates.
