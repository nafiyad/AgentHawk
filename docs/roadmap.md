...
2. Claude Code `PreToolUse` adapter and fixtures. [ADR 0015](adr/0015-claude-code-hook-edge.md) completes the exact-contract research proposal. The closed fixture edge is implemented with a dedicated binary, strict `Bash`/`PowerShell` framing, deny-only output, adversarial process tests, and packed-consumer verification. Configuration ownership and real-host evidence remain separate gates.
...
Milestone 16 is complete. Milestone 17's vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, and unsupported Codex compatibility candidate are complete. [ADR 0015](adr/0015-claude-code-hook-edge.md) pins the independent Claude Code contract to official documentation and exact release `2.1.241`. The closed Claude fixture edge is now implemented: one dedicated entrypoint accepts only bounded fatal-UTF-8 `PreToolUse` objects for canonical `Bash` or `PowerShell`, maps them to separate dialects, discards private framing data, emits only silence, structured deny, or constant exit-2 emergency denial, and is covered by golden, adversarial, process, timeout, no-provider, and packed-consumer tests. It does not read settings or environment values, install configuration, infer deployment trust, or create native support evidence. Every native row remains unsupported. The next dependency-ordered slice is the Claude settings collision/ownership ADR and read-only status design; mutation, activation, and real-host matrices remain later independent gates.
