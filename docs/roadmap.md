...
Milestone 16 is complete. Milestone 17's vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, root-bound Codex project-hook lifecycle, interruption recovery, and invocation-time verification are complete. The exact official Codex CLI `0.149.0` Windows x64 artifact now passes the ordinary project-hook matrix through `codex exec`: unrelated execution makes zero provider calls; allow reaches only a fake package-manager marker; warn, review, block, and strict required-provider error deny with distinct bounded messages; malformed input takes the constant emergency denial; the enabled tool inventory is bound to `shell_command` with Code Mode paths absent; and exact removal succeeds. The same gate records each host scenario's live-evidence interval and enforces controlled nearest-rank p95 targets over 1,000 unrelated qualifications, 25 fresh cache hits, and 25 loopback live-evidence evaluations. The harness uses temporary Git and Codex roots plus loopback-only model/provider fixtures and never runs a real package manager or package code. [ADR 0014](adr/0014-codex-support-scope.md) therefore supports only this exact local project-owned row. App-server, IDE, desktop, Remote, cloud, managed-only, elevated sandbox, Code Mode, Linux, macOS, other versions, and other vendors remain separate unsupported rows. The next dependency-ordered Milestone 17 slice is Claude Code research and an exact edge proposal; implementation cannot inherit Codex payload, authority, timeout, or warning semantics.
