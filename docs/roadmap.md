...
Milestone 16 is complete. Milestone 17's vendor-neutral contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, unsupported Codex compatibility candidate, closed Claude `2.1.241` fixture edge, read-only Claude project-settings preflight, and root-bound lifecycle design are complete. `agenthawk integrations claude status` observes two fixed settings targets, bounded Git topology, and only the quiet ignore exit state through matching snapshots; it reports minimum-disclosure collision states and always keeps activation `unproven`. It never mutates settings, parses foreign local settings, or creates a support claim. ADR 0017 orders the remaining Claude work: pure settings/receipt/root-binding format, invocation verification, receipt-aware status, transaction implementation, then independent real-host support evidence. The next slice is the pure format and known-vector boundary; no install/remove implementation is yet authorized.
