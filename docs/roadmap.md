...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, [Codex project-hook ownership decision](adr/0013-codex-project-hook-ownership.md), root-bound artifact format, and fixed-target read-only `integrations codex status` are complete. Status proves bounded ownership/readiness/blocker observation across ordinary roots, linked worktrees, submodules, and main worktrees with separate Git directories; it does not write, activate, or trust configuration. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate otherwise retains its dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and development-only isolated real-host harnesses. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact official Windows artifact passes both the local CLI and local app-server stdio `shell_command` surfaces. An integrity-pinned Linux x64 Docker attempt failed closed because Codex's bundled `bwrap` could not create its sandbox namespace; no security boundary was weakened. IDE, desktop, Remote, cloud, managed requirements, macOS, other versions, and other vendors remain unproven. The active action is one coherent project-hook lifecycle slice: collision-safe `install`, exact-owned `remove`/partial-state recovery, and invocation-time root-bound pair verification. It must use a capability-tested no-replace publisher, repeat authority/topology/target identities under its lock, preserve receipt-first/hook-second and hook-first/receipt-second ordering, and never claim filesystem locality, power-loss durability, host activation, or support merely from publication. Exact-version manual trust/mutation/disabled/managed-only activation remains the following slice. Do not begin Claude Code or describe Codex native enforcement as supported until those gates are independently reviewed and tested.
