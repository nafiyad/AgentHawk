...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, and bounded aggregate evaluator are complete. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate now has a dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and development-only isolated real-host harnesses. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact official Windows artifact passes both the local CLI and local app-server stdio `shell_command` surfaces: neutral execution has independent regular-file proof and denial does not execute the fake package manager. The app-server harness uses exact-hash hook trust and model-driven turns; it does not use the broad hook-trust flag or unsandboxed command RPCs. An integrity-pinned Linux x64 attempt inside a digest-pinned Docker image failed closed because Codex's bundled `bwrap` could not create its sandbox namespace; no container or Codex security boundary was weakened, so Linux remains unproven. IDE, desktop, Remote, cloud, managed requirements, macOS, other versions, and other vendors remain unproven. The immediate next action is a separate configuration-ownership, collision-safe installation, removal, and recovery decision for the proven Codex surface. Do not begin Claude Code or describe Codex native enforcement as supported until that boundary is reviewed and tested.
