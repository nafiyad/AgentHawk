...
Milestone 16 is complete. Milestone 17's research gate, vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, [Codex project-hook ownership decision](adr/0013-codex-project-hook-ownership.md), root-bound artifact format, fixed-target status, collision-safe install, exact-owned removal, interruption recovery, and invocation-time pair verification are complete. The Codex `rust-v0.149.0` `PreToolUse` compatibility candidate retains its dedicated packaged binary, strict 64 KiB framing, release-pinned golden fixtures, neutral/deny-only bounded output, constant exit-2 emergency denial, process tests, packed-consumer verification, and development-only isolated real-host harnesses. With the documented unelevated backend selected while retaining `workspace-write` and `approval_policy = "never"`, the exact official Windows artifact passes both the local CLI and local app-server stdio `shell_command` surfaces. The isolated project-hook harness now uses the real installer and proves installed-untrusted discovery, exact-definition trust, neutral execution, dependency-add denial with no fake package-manager execution, mutation to Codex `modified` and AgentHawk `owned_modified`, and feature-disabled empty discovery with zero provider traffic. A checksum-verified official `0.149.0` Windows package passed this slice. Managed-only rejection remains unverified because the shipped Windows binary reads the machine-wide requirements location and exposes temporary-path overrides only in debug/test code; the harness does not mutate workstation policy or substitute a debug build. An integrity-pinned Linux x64 Docker attempt also failed closed because Codex's bundled `bwrap` could not create its sandbox namespace; no security boundary was weakened. IDE, desktop, Remote, cloud, managed requirements, macOS, other versions, and other vendors remain unproven. The next dependency-ordered slice must establish managed-only exact-artifact evidence in a disposable environment or make a separate reviewed support-scope decision. Do not begin Claude Code or describe Codex native enforcement as supported until that gate is independently reviewed and tested.
