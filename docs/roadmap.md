...
Milestone 16 is complete. Milestone 17's vendor-neutral action/decision contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, root-bound Codex project-hook lifecycle, interruption recovery, and invocation-time verification are complete. A local standard-user run of the exact Codex CLI `0.149.0` Windows x64 artifact passes the ordinary project-hook matrix through `codex exec`: unrelated execution makes zero provider calls; allow reaches only a fake package-manager marker; warn, review, block, and strict required-provider error deny with distinct bounded messages; malformed input takes the constant emergency denial; the enabled tool inventory is bound to `shell_command` with Code Mode paths absent; and exact removal succeeds. It also records per-host live intervals and controlled nearest-rank p95 over 1,000 unrelated qualifications, 25 fresh cache hits, and 25 loopback live-evidence evaluations. The checksum-pinned GitHub-hosted Windows runner is administrator-elevated, but the observed exact failure is the restricted-token sandbox refusing a split writable-root projection before the outcome matrix. The hosted gate pins the normalized output digest and tagged-source reason, requires true neutral-marker absence, observes zero provider requests, and verifies exact removal; it does not claim a generic administrator rejection. AgentHawk does not weaken the sandbox. [ADR 0014](adr/0014-codex-support-scope.md) therefore keeps every native adapter unsupported and explicitly closes this Codex support attempt pending a reproducible fresh standard-user runner. The next dependency-ordered Milestone 17 slice is Claude Code research and an exact edge proposal; it inherits no Codex payload, authority, timeout, warning, or support evidence.
