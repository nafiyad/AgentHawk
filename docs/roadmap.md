...
2. Claude Code `PreToolUse` adapter and fixtures. [ADR 0015](adr/0015-claude-code-hook-edge.md) completes the exact-contract research proposal. The closed fixture edge is implemented with a dedicated binary, strict `Bash`/`PowerShell` framing, deny-only output, adversarial process tests, and packed-consumer verification. [ADR 0016](adr/0016-claude-project-hook-ownership.md) and `integrations claude status` add only the read-only project-settings collision preflight. [ADR 0017](adr/0017-claude-project-hook-transaction.md) fixes the root-bound receipt, generated settings, invocation-verification, transaction, recovery, and support boundaries; its pure canonical format and invocation-verification slices are implemented, while receipt-aware status, configuration mutation, and real-host evidence remain separate dependency-ordered gates.
...
Milestone 16 is complete. Milestone 17's vendor-neutral contracts, conservative qualifier, typed cancellation, co-root authority, bounded aggregate evaluator, unsupported Codex compatibility candidate, closed Claude `2.1.241` fixture edge, read-only Claude project-settings preflight, root-bound lifecycle design, pure canonical settings/receipt/root-binding format, and invocation-time project verification are complete. The binary accepts only zero launch arguments for the existing fixture boundary or the exact six-argument project declaration; project trust is assigned only after the root-bound pair, absent lock, current runtime artifacts, and matching identity-fenced snapshots verify. Failure returns the constant emergency denial before providers. `agenthawk integrations claude status` still reports only minimum-disclosure collision observations and activation `unproven`; it never mutates settings, parses foreign local settings, or creates a support claim. ADR 0017 orders the remaining Claude work: receipt-aware status, transaction implementation, then independent real-host support evidence. The next slice is receipt-aware read-only status and its minimum-disclosure schema; no install/remove implementation is yet authorized.
