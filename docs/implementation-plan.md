...
Milestone 16 is complete. Its reviewed slices add `policy validate`, `approvals verify`, the Node 22/24 six-platform quality matrix, strict minimum-disclosure `doctor`, and collision-safe `init` with canonical policy discovery and packaged deterministic assets. Milestone 17's research gate is recorded in [ADR 0012](adr/0012-native-hook-enforcement-boundary.md) and the extended threat model. The vendor-neutral slice adds strict bounded action/decision v1 schemas and a pure, flag-free, registry-only POSIX npm/pnpm add qualifier with an adversarial corpus. Before a Codex adapter begins, the next slice must implement the shared propagated deadline/cancellation boundary and co-root default policy/approval resolution required by ADR 0012; no current command or integration is described as native enforcement.
