...
Milestone 16 is complete. Its reviewed slices add `policy validate`, `approvals verify`, the Node 22/24 six-platform quality matrix, strict minimum-disclosure `doctor`, and collision-safe `init` with canonical policy discovery and packaged deterministic assets. Milestone 17's research gate is recorded in [ADR 0012](adr/0012-native-hook-enforcement-boundary.md) and the extended threat model. The vendor-neutral slice adds strict bounded action/decision v1 schemas and a pure, flag-free, registry-only POSIX npm/pnpm add qualifier with an adversarial corpus. The shared operation context propagates typed redacted cancellation through HTTP, retries, npm/OSV, cache, check, Git, scan, and diff while awaiting owned cleanup. The co-root authority prerequisite now establishes one canonical Git-root identity and loads only its default policy, approvals, and optional manifest with bounded identity-fenced reads. Before a Codex adapter begins, the next independent slice is the adapter-owned aggregate deadline and bounded multi-operand evaluation harness. No current command or integration is described as native enforcement.
