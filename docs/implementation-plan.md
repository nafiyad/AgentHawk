...
[ADR 0013](adr/0013-codex-project-hook-ownership.md) now accepts the separate project-hook ownership boundary. The next implementation slice is limited to `agenthawk integrations codex status|install|remove`, one strict paired receipt, fixed contained targets, exclusive collision refusal, receipt-first/hook-second installation, hook-first/receipt-second removal, interruption recovery, redacted state output, and a temporary exact-version project-hook activation harness. It must refuse existing project `hooks.json` or `config.toml`, linked/aliased/mutating targets, foreign or drifted state, arbitrary paths, merge/adopt/force/trust operations, and runtime selection through `PATH` or a package-manager launcher. Acceptance requires adversarial cancellation and identity tests, packaged-consumer failure proof, supported-filesystem no-replace proof, manual exact-definition trust and mutation evidence, disabled/managed-only evidence, the full quality gate, and independent exact-head review. The candidate remains unsupported until those tests pass; user, managed, plugin, session, portable committed, other-version, other-surface, and other-vendor installation remain deferred.
