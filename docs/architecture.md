...
[ADR 0012](adr/0012-native-hook-enforcement-boundary.md) defines the Milestone 17 pre-tool boundary. The core exports strict bounded `AgentAction` and `AgentDecision` v1 schemas plus a pure command qualifier. The action contract retains only adapter identity/version, declared deployment trust, validated root/action paths, grammar classification, and the transient command required for qualification. The decision is a cross-field-validated neutral/deny union with fixed redacted messages; it never contains the raw command or paths. A dedicated `agenthawk-codex-pretooluse` binary implements the release-pinned Codex `rust-v0.149.0` `PreToolUse` `Bash` edge with an explicit restricted `portable` grammar. AgentHawk can collision-safely install and exactly remove its root-bound project-hook pair. Isolated exact-version harnesses prove project discovery, exact trust, a complete local ordinary-CLI allow/warn/review/block/error matrix, unrelated zero-provider behavior, malformed denial, enabled-tool binding, mutation, feature-disable behavior, and managed-only suppression. The pinned hosted Windows environment instead rejects the restricted-token split writable-root projection and is retained as exact exclusion evidence. [ADR 0014](adr/0014-codex-support-scope.md) therefore keeps the candidate unsupported. Evidence cannot be generalized across operating systems, clients, execution environments, deployment authorities, tool configurations, or versions.
...
[ADR 0015](adr/0015-claude-code-hook-edge.md) defines the independent Claude Code boundary. The dedicated `agenthawk-claude-pretooluse` fixture edge pins `2.1.241`, distinguishes canonical `Bash` from `PowerShell`, discards all private framing fields, and emits only neutral silence, structured deny, or constant exit-2 emergency denial. The current PowerShell qualifier conservatively classifies every PowerShell command as unsupported, so each denies without provider access; no PowerShell dependency addition is admitted. The edge neither reads nor mutates Claude settings, and deployment trust remains `unknown`. Claude command-hook startup, malformed-output, and timeout failures are host-side non-blocking residuals; no Claude native row is supported.

[ADR 0016](adr/0016-claude-project-hook-ownership.md) defines and now governs the implemented Claude collision preflight without mutation. A future machine-local candidate may exclusively own only a previously absent `.claude/settings.local.json` whose exact path passes a quiet shell-free Git ignore check; shareable `.claude/settings.json` stays maintainer-owned and read-only. Status establishes only canonical root identity—it does not load policy, approvals, or the repository manifest—then directly opens the two fixed settings files and runs fixed shell-free Git topology plus exit-code-only quiet-ignore queries in two matching snapshots. Git may consult its applicable repository, parent, per-repository, and global exclude sources, but AgentHawk retains only bounded topology and ignore states and no pattern, source path, or bytes. The strict report exposes five observations, ordered blockers, fixed activation `unproven`, and a stable exit meaning. Shared settings use fatal UTF-8, duplicate rejection, structural depth/width bounds, and narrow inspection of only `PreToolUse` and `disableAllHooks`; foreign local settings are never parsed. The command reads no user, managed, plugin, session, SDK, command-line, trust, or environment state and makes no ownership, readiness, or effective-activation claim.

[ADR 0017](adr/0017-claude-project-hook-transaction.md) fixes the design for a later exclusive Claude lifecycle without enabling it. Its implemented pure format emits compact deterministic exec-form local settings and a path-redacted closed receipt from canonical absolute Node/adapter paths, a CSPRNG installation identifier, length-framed root identity, and exact artifact digests. Fatal-UTF-8 canonical parsers and fixed Windows/POSIX vectors reject alternate bytes, duplicate or unknown data, unsafe paths, controls, invalid identity, and framing ambiguity. The implemented invocation verifier accepts only the exact six-argument declaration, re-establishes authority from bounded hook input, observes the fixed pair and lock twice through identity fences, recomputes current runtime artifacts, and assigns project deployment trust only after an exact match; failure uses the constant emergency denial before providers. Neither format nor invocation is wired to status or a mutation command. Future mutation must use a fixed exclusive lock, verified staging files, an actual-filesystem no-replace capability probe, receipt-first install, settings-first remove, explicit cancellation linearization, and fixed-target non-recursive recovery. Status and host activation remain independent, and every Claude native support row remains unsupported.
