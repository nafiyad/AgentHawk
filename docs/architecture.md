...
AgentHawk `allow` maps to a neutral host result, not host permission. `warn` can remain neutral only through a tested vendor channel that visibly surfaces it without granting permission; otherwise it must deny for review. Review, block, error, unsupported dependency-like input, and internal failure must map to denial while the adapter can still respond. The core operation context now carries typed redacted cancellation through HTTP attempts, retry waits, npm/OSV providers, cache operations, check orchestration, and bounded Git; caller cancellation is not retried or converted into ordinary provider evidence. Scan and diff await all work they start before returning from a concurrent failure. A future adapter still must own the aggregate deadline, apply it to co-root discovery and bounded multi-operand evaluation, and prove end-to-end quiescence before serialization. An outermost constant exit-2 path must deny independently of the normal serializer. Host timeout semantics remain external: Codex has no documented fail-closed timeout guarantee, Claude Code command-hook timeouts proceed normally, Cursor requires `failClosed: true`, and GitHub Copilot timeouts are documented fail-open. Cursor's current contract has no documented neutral/defer result, so it is unsupported unless a real version-pinned harness proves neutral behavior without emitting host allow. Copilot cloud does not assume npm/OSV network access and has no user-visible warning channel; missing provider allowance or a warn result denies. Native hooks therefore remain a version- and deployment-specific guardrail; protected CI independently re-evaluates repository changes.
