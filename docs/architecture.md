...
[ADR 0016](adr/0016-claude-project-hook-ownership.md) defines the next Claude collision boundary without mutation. A future machine-local candidate may exclusively own only a previously absent `.claude/settings.local.json` whose exact path passes a quiet shell-free Git ignore check; shareable `.claude/settings.json` stays maintainer-owned and read-only. Before a root-bound receipt format exists, status directly opens only those two fixed settings files and runs fixed shell-free Git topology and quiet-ignore queries. Git may consult its applicable repository, parent, per-repository, and global exclude sources, but AgentHawk retains only bounded topology and ignore states and no pattern, source path, or bytes. Stable observations report local/shared presence, the ignore precondition, shared `PreToolUse` and `disableAllHooks` declarations, deterministic unsafe blockers, and activation `unproven`. It directly reads no user, managed, plugin, session, SDK, command-line, trust, or environment state and makes no ownership, readiness, or effective-activation claim.
