...
## Native hook decision boundary (planned)

[ADR 0012](adr/0012-native-hook-enforcement-boundary.md) defines the Milestone 17 pre-tool boundary. The core now exports strict bounded `AgentAction` and `AgentDecision` v1 schemas plus a pure command qualifier. The action contract retains only adapter identity/version, declared deployment trust, validated root/action paths, shell dialect, and the transient command required for qualification. The decision is a cross-field-validated neutral/deny union with fixed redacted messages; it never contains the raw command or paths. Each host still requires its own strict edge schema and serializer pinned by reviewed documentation, fixtures, and a real-host compatibility harness; no native adapter is implemented or supported yet.

The pure v1 qualifier recognizes only direct lowercase POSIX `npm install|i|add` and `pnpm add` commands with one to eight unquoted, flag-free registry operands separated by ASCII spaces. It validates every operand with the production npm-spec parser and returns only bounded normalized coordinates in input order. Only lexically simple commands with no manager-like token are `unrelated`. A manager token in any other position, recognized wrappers/interpreters, assignments, reserved shell forms, composition, substitution, quoting, escaping, glob syntax, ephemeral execution, manager flags, path-qualified or mixed-case executables, workspace/non-registry operands, unsupported dialects, and malformed input receive explicit non-evaluated classifications—even when a command merely mentions a manager. Qualification performs no policy, cache, provider, Git, package-manager, or shell operation. Unknown alias/function/script bodies and commands that expose no manager token or never reach a supported hook remain outside this string-level boundary and are compensated by protected `scan`/`diff` CI.

Vendor `cwd` is not trusted as a repository root. The internal repository-authority loader now canonicalizes an absolute action directory, discovers its Git worktree root through the hardened argument-array Git boundary, requires exact canonical-root and observed filesystem-identity agreement, and reads only the co-rooted default policy, approvals, and optional manifest. It rejects nested action directories, observed symbolic redirection, unsafe or changing files, missing repository identity, and malformed configuration; optional files are rechecked before defaults are accepted. Vendor payloads cannot select configuration paths. This prerequisite is not wired to a hook yet, and standalone CLI explicit-path behavior is unchanged.

AgentHawk `allow` maps to a neutral host result, not host permission. `warn` can remain neutral only through a tested vendor channel that visibly surfaces it without granting permission; otherwise it must deny for review. Review, block, error, unsupported dependency-like input, and internal failure map to denial while the adapter can still respond. The adapter-neutral evaluation harness consumes one fixed eight-second deadline owned by its future adapter edge, loads repository authority once, evaluates at most eight coordinates with a two-worker pool, deduplicates exact coordinates for provider work, restores original operand order, combines verdicts deterministically, and waits for all started work before returning. The core operation context carries typed redacted cancellation through HTTP attempts, retry waits, npm/OSV providers, cache operations, check orchestration, and bounded Git; caller cancellation is not retried or converted into ordinary provider evidence. A future adapter must create and dispose the deadline at its input edge, serialize exactly one bounded decision after quiescence, and provide an outermost constant exit-2 denial independent of the normal serializer. Host timeout semantics remain external: Codex has no documented fail-closed timeout guarantee, Claude Code command-hook timeouts proceed normally, Cursor requires `failClosed: true`, and GitHub Copilot timeouts are documented fail-open. Cursor's current contract has no documented neutral/defer result, so it is unsupported unless a real version-pinned harness proves neutral behavior without emitting host allow. Copilot cloud does not assume npm/OSV network access and has no user-visible warning channel; missing provider allowance or a warn result denies. Native hooks therefore remain a version- and deployment-specific guardrail; protected CI independently re-evaluates repository changes. No native adapter is implemented or supported yet.
