...
[ADR 0012](adr/0012-native-hook-enforcement-boundary.md) defines the Milestone 17 pre-tool boundary before implementation. Each host requires its own strict edge schema and serializer pinned by reviewed documentation, fixtures, and a real-host compatibility harness; this is not runtime host-version authentication. A bounded vendor-neutral action and decision contract will sit between that edge and the existing npm parser, providers, policy, approvals, cache, and verdict precedence. Raw vendor payloads must never enter the policy engine.

The first adapter will qualify only direct, fixture-backed npm/pnpm dependency-add command shapes at the validated Git root. Unrelated commands must produce a neutral result with zero provider calls. Dependency-like commands that are malformed, nested below the root, workspace-targeted, composed through unsupported shell syntax, hidden behind wrappers, ephemeral executions, or otherwise ambiguous must be denied without being described as analyzed. The qualifier must never invoke a package manager or reconstruct a shell command.
