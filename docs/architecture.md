...
[ADR 0016](adr/0016-claude-project-hook-ownership.md) defines the Claude collision preflight without mutation. A machine-local candidate may exclusively own only a previously absent `.claude/settings.local.json` whose exact path passes a quiet shell-free Git ignore check; shareable `.claude/settings.json` stays maintainer-owned and read-only. Status establishes canonical root identity without loading policy, approvals, or the repository manifest, opens only fixed targets, and runs fixed shell-free Git topology plus exit-code-only quiet-ignore queries in two matching snapshots. Git may consult its applicable repository, parent, per-repository, and global exclude sources, but AgentHawk retains only bounded topology and ignore states and no pattern, source path, or bytes. Shared settings use fatal UTF-8, duplicate rejection, structural depth/width bounds, and narrow inspection of only `PreToolUse` and `disableAllHooks`; foreign local settings are never parsed. User, managed, plugin, session, SDK, command-line, trust, and environment state remain outside observation.

[ADR 0017](adr/0017-claude-project-hook-transaction.md) fixes the design for a later exclusive Claude lifecycle without enabling it. Its implemented pure format emits compact deterministic exec-form local settings and a path-redacted closed receipt from canonical absolute Node/adapter paths, a CSPRNG installation identifier, length-framed root identity, and exact artifact digests. Fatal-UTF-8 canonical parsers and fixed Windows/POSIX vectors reject alternate bytes, duplicate or unknown data, unsafe paths, controls, invalid identity, and framing ambiguity. The invocation verifier accepts only the exact six-argument declaration, re-establishes authority from bounded hook input, observes the fixed pair and lock twice through identity fences, recomputes current runtime artifacts, and assigns project deployment trust only after an exact match; failure uses the constant emergency denial before providers. Receipt-aware status validates the same pair, classifies ownership independently from current artifact readiness, observes the canonical lock and only its derived staging target, and emits aggregate ignore state without disclosing ownership material. No mutation command exists. Future mutation must use a fixed exclusive lock, verified staging files, an actual-filesystem no-replace capability probe, receipt-first install, settings-first remove, explicit cancellation linearization, and fixed-target non-recursive recovery. Status and host activation remain independent, and every Claude native support row remains unsupported.
