...
[ADR 0016](adr/0016-claude-project-hook-ownership.md) defines the next Claude collision boundary without mutation. A future machine-local candidate may exclusively own only a previously absent `.claude/settings.local.json`; shareable `.claude/settings.json` stays maintainer-owned and read-only. Before a root-bound receipt format exists, status is only a two-file preflight: bounded stable observations report local/shared presence, shared `PreToolUse` and `disableAllHooks` declarations, fixed blockers, and activation `unproven`. It reads no user, managed, plugin, session, SDK, command-line, trust, or environment state and makes no ownership, readiness, or effective-activation claim.
