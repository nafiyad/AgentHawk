...
[ADR 0015](adr/0015-claude-code-hook-edge.md) defines the independent Claude Code research boundary. The proposed `2.1.241` edge must distinguish canonical `Bash` from `PowerShell`, emit only neutral silence or deny, and treat project settings as untrusted executable configuration. The current PowerShell qualifier admits no dependency additions, so Windows admission stays unavailable until a separate restricted grammar passes. Claude command-hook startup, malformed-output, and timeout failures are non-blocking; no Claude native row is supported.
