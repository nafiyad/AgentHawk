...
The pure v1 qualifier recognizes only direct lowercase POSIX `npm install|i|add` and `pnpm add` commands with one to eight unquoted, flag-free registry operands separated by ASCII spaces. It validates every operand with the production npm-spec parser and returns only bounded normalized coordinates in input order. Only lexically simple non-manager commands are `unrelated`. Recognized wrappers/interpreters, assignments, reserved shell forms, composition, substitution, quoting, escaping, glob syntax, ephemeral execution, manager flags, path-qualified or mixed-case executables, workspace/non-registry operands, unsupported dialects, and malformed input receive explicit non-evaluated classifications—even when a complex command might be benign. Qualification performs no policy, cache, provider, Git, package-manager, or shell operation. Unknown alias/function/script bodies and commands that never reach a supported hook remain outside this string-level boundary and are compensated by protected `scan`/`diff` CI.
