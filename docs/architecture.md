...
Vendor `cwd` is not trusted as a repository root. The initial adapter must canonicalize the action directory, discover one containing Git worktree root through the hardened argument-array Git boundary, require the two to match until workspace-aware admission exists, and anchor default policy, approvals, and manifest context to that root. Vendor payloads cannot select configuration paths. A failure to establish one consistent repository identity must deny a dependency-like action.
