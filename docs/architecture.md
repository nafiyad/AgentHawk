...
[ADR 0017](adr/0017-claude-project-hook-transaction.md) fixes the design for a later exclusive Claude lifecycle without enabling it. Canonical local-settings bytes use exec-form absolute Node and packaged-adapter paths plus a CSPRNG installation identifier and path-free root binding. A closed receipt binds those bytes and current artifacts to one canonical root. Future mutation must use a fixed exclusive lock, verified staging files, an actual-filesystem no-replace capability probe, receipt-first install, settings-first remove, explicit cancellation linearization, and fixed-target non-recursive recovery. Invocation must revalidate the exact pair before deployment trust may become `project`; status and host activation remain independent, and every Claude native support row remains unsupported.
