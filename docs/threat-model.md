...
| Hook `cwd`, policy, approvals, and manifest resolve from different roots | The internal authority loader requires the canonical action directory to equal the discovered Git worktree root, uses only co-rooted defaults, rechecks optional absence and root identity, and rejects nested/workspace-targeted actions; no hook consumes it yet | PATH-selected local Git, filesystem-specific identity quality, unsupported path aliases, and same-account races remain in the boundary |
...
| Hook failure, serializer failure, or timeout becomes an advertised fail-closed control | The shared typed cancellation context reaches Git, check, HTTP/retry/pagination, and cache operations without retrying caller cancellation. The adapter-neutral harness uses one fixed eight-second deadline, a two-worker pool for at most eight operands, first-cause cancellation, deterministic aggregation, and complete settlement of started work. Each vendor edge must still create and dispose that deadline, prove bounded framing, and add a constant outermost exit-2 emergency denial independent of normal serialization; document vendor timeout behavior and require Cursor `failClosed: true` | Filesystem cancellation is best effort, remote provider computation and universal process-tree termination are not guaranteed, and a process that cannot start, a host timeout, or a bypassed hook can still proceed; protected CI remains final |
