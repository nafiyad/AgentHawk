...
- `agenthawk init` creates deterministic policy and selected advisory integration files with collision-safe, no-overwrite publication and exact idempotency.
...
- `check` and `scan` load canonical optional `.agenthawk.yml` by default; an explicit `--policy` path retains precedence.
